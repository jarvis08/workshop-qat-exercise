...
// Quantization, INT4 data type
typedef struct {
    unsigned char el : 4;
} int4_t;

typedef struct {
    int size;  // number of data in cluster
    int idx;   // current index to train
    int done;  // to check if trained 1 epoch
    int *map;  // randomly shuffled indices of its cluster
    float min; // for ema
    float max; // for ema
} cluster;
...
struct layer{
    ...
    // quantization cpu
    void (*forward_int4)   (struct layer, struct network, int dequant);
    void (*forward_int8)   (struct layer, struct network, int dequant);
    void (*forward_qparams)   (struct layer, struct network);
    void (*forward_cmp)   (struct layer, struct network, int dequant);
    void (*forward_csv)   (struct layer, struct network, int dequant, FILE *output_csv, FILE *weight_csv);
    void (*forward_qat_int8)   (struct layer, struct network);

    // quantization gpu
    void (*forward_bmc_int4_gpu)   (struct layer, struct network);
    void (*forward_int4_gpu)   (struct layer, struct network, int dequant);
    void (*forward_int8_gpu)   (struct layer, struct network, int dequant);
    void (*forward_qat_int4_gpu)   (struct layer, struct network);
    void (*forward_qat_int8_gpu)   (struct layer, struct network);
    void (*forward_qat_cluster_int4_gpu)   (struct layer, struct network, const int cluster);
    void (*forward_qat_cluster_int8_gpu)   (struct layer, struct network, const int cluster);
    void (*forward_ema_gpu)   (struct layer, struct network);
    void (*forward_qparams_gpu)   (struct layer, struct network);
    ...
    size_t workspace_int4_size;
    size_t workspace_int8_size;

#ifdef GPU
    float* weight_velocity_gpu;
    float* bias_velocity_gpu;
    
    // quantization
    float * fake_int_weights_gpu;
    int8_t *weights_int8_gpu;      // cuda allocate
    ...
    float * norms_gpu;
#ifdef CUDNN
    void (*forward_cudnn_gpu)   (struct layer, struct network, int dequant);
    size_t workspace_cudnn_size;
...
typedef struct network{
    ...
    int qat_end_epoch;
    float ema_smooth;
    float ema_convergence;
    int ema_decay;
    float ema_decay_factor;
    int *ema_init;

    // to make input sequentially
    int cur_idx;   // data index to train (load_args.cur_idx = &net->cur_idx)
    int ctr_k;     // k value
    char *ctr_method;
    int *ctr_info;
    ...
    int8_t *input_int8;
    int8_t *output_int8;
    int8_t *workspace_int8;
    int4_t *workspace_int4;
...
#ifdef GPU
    // quantization
    int8_t *workspace_int8_gpu;
    int8_t *input_int8_gpu;
    int8_t *output_int8_gpu;

    // INT4
    int4_t *input_int4_gpu;
    int4_t *output_int4_gpu;
...
typedef struct load_args{
    ...
    int* data_map;
    int* cur_idx;
    cluster *clusters; // cluster structs array 
...
void save_converted_weight(network *net, char *filename);
void load_weights_from_txt(network *net, char *filename);
void load_weights_upto_tf(network *net, char *filename, int start, int cutoff);
...
void load_only_quantized_weights(network *net, char *filename_qw, char *filename_qsz, int start, int cutoff);
void load_quantized_weights(network *net, char *filename, char *filename_qw, char *filename_qsz);
...
void load_quantized_weights_upto(network *net, char *filename, char *filename_qw, char *filename_qsz, int start, int cutoff);
void load_weights_for_transfer_learning(network *net, char *filename);
void save_weights_quant(network *net, char *filename, char *filename_qw, char *filename_qsz);
void save_weights_upto_quant(network *net, char *filename, char *filename_qw, char *filename_qsz, int cutoff);
...
void forward_network_int8(network *net);
void forward_quantization_simulated_network_int8(network *net);
void forward_network_comparison(network *net);
void forward_network_quant_csv(network *net, FILE *output_csv, FILE *weight_csv);
...
void forward_network_qparams_staged(network *net, int stage);
...
// quantization gpu ====================================================================================================
void forward_network_int4_gpu(network *net);
void forward_network_int8_gpu(network *net);
void forward_simulated_quantization_network_int4_gpu(network *net);
void forward_simulated_quantization_network_int8_gpu(network *net);
...
void forward_network_qparams_gpu(network *net);
void forward_network_batch_qparams_gpu(network *net);
void forward_network_qparams_staged_gpu(network *net, int stage);

// blas_kernels.cu
void fill_gpu_int(int N, int ALPHA, int * X, int INCX);
void fill_gpu_int8(int N, int8_t ALPHA, int8_t * X, int INCX);
void fill_gpu_int32(int N, int32_t ALPHA, int32_t * X, int INCX);
// =====================================================================================================================

// Fine-tuning
void mask_network_gpu(network *net);
void backward_tuned_network_gpu(network *net);
...
image load_image_without_resizing(char *filename, int c);
image load_image_color_without_resizing(char *filename);
...
pthread_t load_data_sequence_in_thread_without_preprocessing(load_args args);
