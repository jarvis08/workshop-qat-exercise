...
def load_npy(path):
    train = np.load(os.path.join(path, "train_minmax.npy"))
    test = np.load(os.path.join(path, "test_minmax.npy"))
    return train, test
...
print("Save model")
joblib.dump(kmeans, os.path.join(DATASET, 'minmax.pkl')) 

print("Save as txt file")
np.savetxt(os.path.join(DATASET, 'train_minmax.txt'), kmeans.labels_, delimiter=',', fmt='%i')
np.savetxt(os.path.join(DATASET, 'test_minmax.txt'), kmeans.predict(test_data), delimiter=',', fmt='%i')
...
plt.savefig(os.path.join(DATASET, 'train_minmax.png'), dpi=500)
plt.clf()
...
plt.xlabel('Min Value')
plt.ylabel('Max Value')
plt.savefig(os.path.join(DATASET, 'test_minmax.png'), dpi=500)
