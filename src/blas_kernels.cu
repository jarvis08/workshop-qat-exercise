...
__global__ void fill_kernel_int(int N, int ALPHA, int *X, int INCX)
{
    int i = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
    if(i < N) X[i*INCX] = ALPHA;
}

__global__ void fill_kernel_int8(int N, __int8_t ALPHA, __int8_t *X, int INCX)
{
    int i = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
    if(i < N) X[i*INCX] = ALPHA;
}

__global__ void fill_kernel_int32(int N, __int32_t ALPHA, __int32_t *X, int INCX)
{
    int i = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
    if(i < N) X[i*INCX] = ALPHA;
}
...
extern "C" void fill_gpu_int(int N, int ALPHA, int * X, int INCX)
{
    fill_kernel_int<<<cuda_gridsize(N), BLOCK>>>(N, ALPHA, X, INCX);
    check_error(cudaPeekAtLastError());
}

extern "C" void fill_gpu_int8(int N, int8_t ALPHA, int8_t * X, int INCX)
{
    fill_kernel_int8<<<cuda_gridsize(N), BLOCK>>>(N, ALPHA, X, INCX);
    check_error(cudaPeekAtLastError());
}

extern "C" void fill_gpu_int32(int N, int32_t ALPHA, int32_t * X, int INCX)
{
    fill_kernel_int32<<<cuda_gridsize(N), BLOCK>>>(N, ALPHA, X, INCX);
    check_error(cudaPeekAtLastError());
}
