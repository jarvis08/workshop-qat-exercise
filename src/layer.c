...
void free_layer(layer l)
{
    ...
    if(l.qs)                 free(l.qs);
    if(l.qz)                 free(l.qz);
    if(l.act_range)          free(l.act_range);
    if(l.act_cluster)        free(l.act_cluster);
    if(l.output_int8)        free(l.output_int8);
    if(l.weights_int8)       free(l.weights_int8);
    if(l.biases_int32)       free(l.biases_int32);
    if(l.fake_int_weights)   free(l.fake_int_weights);
    if(l.ema_init)           free(l.ema_init);

    if(l.qz_int4)            free(l.qz_int4);
    if(l.weights_int4)       free(l.weights_int4);
    if(l.output_int4)        free(l.output_int4);

    // Fine-tuning
    if(l.weights_mask)       free(l.weights_mask);
    if(l.weights_range)      free(l.weights_range);

    // Momentum
    if(l.weight_velocity)    free(l.weight_velocity);
    if(l.bias_velocity)      free(l.bias_velocity);
#ifdef GPU
    ...
    //quantization
    if(l.fake_int_weights_gpu)    cuda_free(l.fake_int_weights_gpu);
    if(l.weights_int8_gpu)        cuda_free_int8(l.weights_int8_gpu);
    if(l.output_int32_gpu)        cuda_free_int32(l.output_int32_gpu);
    if(l.output_int32)            free(l.output_int32);

    if(l.weight_velocity_gpu)     cuda_free(l.weight_velocity_gpu);
    if(l.bias_velocity_gpu)       cuda_free(l.bias_velocity_gpu);
#endif
}
