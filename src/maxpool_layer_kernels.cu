...
__global__ void forward_maxpool_layer_int8_kernel(int n, int in_h, int in_w, int in_c, int stride, int size, int pad, int8_t *input, int8_t *output, int *indexes)
{
    int h = (in_h + pad - size)/stride + 1;
    int w = (in_w + pad - size)/stride + 1;
    int c = in_c;
    ...
    for(l = 0; l < size; ++l){
        for(m = 0; m < size; ++m){
            ...
            int valid = (cur_h >= 0 && cur_h < in_h &&
                    cur_w >= 0 && cur_w < in_w);
            int8_t val = (valid != 0) ? input[index] : -128;
            max_i = (val > max) ? index : max_i;
            max   = (val > max) ? val   : max;
        }
    }
...
extern "C" void forward_maxpool_layer_int8_gpu(maxpool_layer layer, network net, int dequant)
{
    int h = layer.out_h;
    int w = layer.out_w;
    int c = layer.c;
    ...
    forward_maxpool_layer_int8_kernel<<<cuda_gridsize(n), BLOCK>>>(n, layer.h, layer.w, layer.c, layer.stride, layer.size, layer.pad, net.input_int8_gpu, layer.output_int8_gpu, layer.indexes_gpu);
    check_error(cudaPeekAtLastError());
}
