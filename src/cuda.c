...
int8_t *cuda_make_array_int8(int8_t *x, size_t n)
{
    ...
    if(x){
        ...
        fill_gpu_int8(n, 0, x_gpu, 1);
    }
    if(!x_gpu) error("Cuda malloc int8_t failed\n");
    return x_gpu;
}
...
int32_t *cuda_make_array_int32(int32_t *x, size_t n)
{
    ...
    if(x){
        ...
        fill_gpu_int32(n, 0, x_gpu, 1);
    }
    if(!x_gpu) error("Cuda malloc int32_t failed\n");
    return x_gpu;
}
